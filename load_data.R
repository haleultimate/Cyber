...
load_data <- function(cybc.list) {
  ...
  for (cybcstr in cybc.list) {
    ...
    # if (cybcstr == "BTC") {  #needs to be first in cybc.list
    #   data.env$date.xts <- data.env$BTC.xts[,"Date"]
    #   data.env$date_list <- date_list
    # }
    # cmd_str <- paste0("data.env$",cybcstr,".xts <- merge(data.env$date.xts,data.env$",cybcstr,".xts)")
    # print(cmd_str)
    # eval(parse(text=cmd_str))
